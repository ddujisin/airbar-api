...
@pytest.mark.asyncio
async def test_notification_preferences_crud(test_user, preferences_manager):
    ...
    # Save preferences
    saved_prefs = await preferences_manager.update_preferences(test_user, prefs)
    assert saved_prefs.user_id == test_user
    assert saved_prefs.quiet_hours_start == 22

    # Get preferences
    retrieved_prefs = await preferences_manager.get_preferences(test_user)
    assert retrieved_prefs.user_id == test_user
    assert retrieved_prefs.quiet_hours_start == 22

@pytest.mark.asyncio
async def test_notification_quiet_hours(test_user, preferences_manager):
...
@pytest.mark.asyncio
async def test_notification_muting(test_user, preferences_manager):
    """Test notification muting functionality."""
    # Set up initial preferences
    prefs = await preferences_manager.get_preferences(test_user)
    assert NotificationPreferencesManager.should_notify(prefs, "email")

    # Mute notifications
    muted_prefs = await preferences_manager.mute_notifications(test_user, 30)
    assert not NotificationPreferencesManager.should_notify(muted_prefs, "email")

@pytest.mark.asyncio
async def test_priority_threshold(test_user, preferences_manager):
